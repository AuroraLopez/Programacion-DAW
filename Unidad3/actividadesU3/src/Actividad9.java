...
public class Actividad9 {
    public static void main(String[] args) {
        ...
        notas = eliminarProgramacion(materias, notas); // elimina la columna de "Programacion"
        materias = eliminarMateria(materias, "Programacion"); // elimina el nombre de la materia
    ...
    public static double[][] eliminarProgramacion(String[] materias, double[][] notas) {
        int posicion = -1;
        // Buscar la posición de "Programacion"
        for (int i = 0; i < materias.length; i++) {
        ...
        if (posicion == -1)
            return notas; // No existe "Programacion"
    ...
    // Función para eliminar la materia del array de materias
    public static String[] eliminarMateria(String[] materias, String nombreMateria) {
        int posicion = -1;
        for (int i = 0; i < materias.length; i++) {
            if (materias[i].equals(nombreMateria)) {
                posicion = i;
                break;
            }
        }
        ...
        String[] nuevasMaterias = new String[materias.length - 1];
        int k = 0;
        for (int i = 0; i < materias.length; i++) {
            if (i != posicion) {
                nuevasMaterias[k++] = materias[i];
            }
        }
